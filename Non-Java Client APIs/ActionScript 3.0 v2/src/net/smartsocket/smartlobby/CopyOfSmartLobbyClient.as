...
package net.smartsocket.smartlobby {
	...
	import net.smartsocket.SmartLobbyClient;
	 ...
	 public class CopyOfSmartLobbyClient extends SmartSocketClient{
		...
		public function CopyOfSmartLobbyClient() {
			super();
		}
		...
		protected override function onJSON(event:ProgressEvent):void {
			...
			for(var i:Number = 0; i < arr.length; i++) {
				...
				try {
					...
					CopyOfSmartLobbyClient.lobby.dispatchEvent( new SmartLobbyEvent(SmartLobbyEvent[method], params) );
					...
					for(var j:String in CopyOfSmartLobbyClient.customListeners) {
						 
						if(CopyOfSmartLobbyClient.customListeners[j].hasOwnProperty(method)) {
							try {
								trace("SmartLobby => Trying "+method+" on "+CopyOfSmartLobbyClient.customListeners[j]);
								CopyOfSmartLobbyClient.customListeners[j][method](params);
							}catch(e:Error) {
