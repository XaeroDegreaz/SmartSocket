...
\n\n");
	...
	if((string)$xml->auto_update == "false") {
		...
		if($latest = @(int)file_get_contents("http://www.smartsocket.net/autoupdate/BUILD")) {
			//# Compare build number
			if(SMARTSOCKET_BUILD < $latest) {
				Logger::log(__FUNCTION__ , "Update found...");
								
				if($file = @file_get_contents("http://www.smartsocket.net/autoupdate/libsmartsocket.dll", FILE_BINARY)) {
